...
public class EsProductServiceImpl implements EsProductService {
    ...
    public int importAll() {
        ...
        // Bulk indexing optimization: significantly faster than individual saveAll for large datasets
        int batchSize = 1000;
        int total = esProductList.size();
        for (int i = 0; i < total; i += batchSize) {
            int end = Math.min(i + batchSize, total);
            List<EsProduct> batch = esProductList.subList(i, end);
